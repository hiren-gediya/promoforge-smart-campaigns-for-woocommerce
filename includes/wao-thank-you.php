...
add_action('woocommerce_before_thankyou', 'flashoffers_offer_thankyou_total_saving_notice', 5);

// Show total savings on the Thank You page
function flashoffers_offer_thankyou_total_saving_notice($order_id)
{
    global $wpdb;
    if (!$order_id)
        return;
    ...
    foreach ($order->get_items() as $item) {
        ...
        $bogo_offer_data = flashoffers_get_bogo_offer_data($product);
    ...
    if ($total_saved > 0) {
        ...
        $message = !empty($options['message'])
            ? (strpos($options['message'], '{amount}') === false
                ...
                : $options['message'])
            : esc_html__('🎉 Congratulations! You saved {amount} in this order', 'advanced-offers-for-woocommerce');
...
add_filter('woocommerce_order_item_name', 'flashoffers_offer_order_item_price_html', 10, 3);
function flashoffers_offer_order_item_price_html($item_name, $item, $is_visible)
{
    $product = $item->get_product();
    if (!$product)
        return $item_name;
    ...
    if ($flash_offer_data && 'active' === $flash_offer_data['status']) {
        ...
        $price_html = flashoffers_bogo_offers_price_html_override($product->get_price_html(), $product);
    }
