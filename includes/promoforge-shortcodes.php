...
add_shortcode('flash_special_offer', 'promoforge_display_special_offer_products');
...
function promoforge_display_special_offer_products($atts)
{
    ...
    if (!$offer_id)
        return '<p class="flash-offer-error">' . esc_html__('Invalid offer ID.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    ...
    $offer = $wpdb->get_row($wpdb->prepare(
        "SELECT o.id, o.discount, o.end_date FROM {$wpdb->prefix}promoforge_flash_offers o
    ...
    if (!$offer)
        return '<p class="flash-offer-error">' . esc_html__('Invalid or missing special offer.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';

    // Check if offer is expired
    if (strtotime($offer->end_date) < current_time('timestamp')) {
        return '<p class="flash-offer-error">' . esc_html__('This special offer has expired.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    }
    ...
    if (empty($product_ids))
        return '<p class="flash-offer-notice">' . esc_html__('No products found in this offer.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    ...
    if (!$products->have_posts())
        return '<p class="flash-offer-notice">' . esc_html__('No valid products found.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
