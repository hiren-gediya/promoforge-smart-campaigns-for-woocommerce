...
add_shortcode('promoforge_special_offer', 'promoforge_display_special_offer_products');

function promoforge_display_special_offer_products($atts)
{
    $atts = shortcode_atts([
        'id' => 0, // Special Offer post ID
        'columns' => 3,
        'limit' => -1,
    ], $atts, 'promoforge_special_offer');

    $offer_id = (int) $atts['id'];
    if (!$offer_id)
        return '<p class="promoforge-offer-error">' . esc_html__('Invalid offer ID.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    ...
    $offer = $wpdb->get_row($wpdb->prepare(
        "SELECT o.id, o.discount, o.end_date FROM {$wpdb->prefix}promoforge_offers o
    ...
    if (!$offer)
        return '<p class="promoforge-offer-error">' . esc_html__('Invalid or missing special offer.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';

    // Check if offer is expired
    if (strtotime($offer->end_date) < current_time('timestamp')) {
        return '<p class="promoforge-offer-error">' . esc_html__('This special offer has expired.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    }
    ...
    if (empty($product_ids))
        return '<p class="promoforge-offer-notice">' . esc_html__('No products found in this offer.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    ...
    if (!$products->have_posts())
        return '<p class="promoforge-offer-notice">' . esc_html__('No valid products found.', 'promoforge-smart-campaigns-for-woocommerce') . '</p>';
    ...
    $slider_class = 'promoforge-offer-slider-enabled';

    // Output
    ob_start();
    ?>

    <div class="woocommerce promoforge-special-offer-container <?php echo esc_attr($slider_class); ?>"
        data-offer-id="<?php echo (int) $offer_id; ?>" data-columns="<?php echo (int) $atts['columns']; ?>">
        <div class="promoforge-products">
            <?php
            $GLOBALS['promoforge_special_offer_context'] = true;
            while ($products->have_posts()):
                ...
                <div class="promoforge-product promoforge-special-offer-product product">
                    ...
                    <div class="promoforge-special-offer-actions">
                        <?php
            ...
            <?php endwhile;
            unset($GLOBALS['promoforge_special_offer_context']);
            ?>
        </div>
    </div>
